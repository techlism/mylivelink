...
export async function POST(req: NextRequest) {
  const { link } : {link : LinkSchema} = await req.json();
  // if(data){
  //     const {link} : {link : LinkSchema} = await bodyParser(data);
  if (link) {
    try {
      const res = await dbExecute(
        `UPDATE Links SET title = ?, url = ?, thumbnailUrl = ?, type = ?, faviconURL = ?, category = ? WHERE username = ?`,
        [
          link.title,
          link.url,
          link.thumbnailUrl,
      ...
      if (res.rowsAffected > 0)
        return NextResponse.json({ success: "link updated" });
      else return NextResponse.json({ failure: "unable to modify link" });
    } catch (error) {
      return NextResponse.json({ error: error });
    }
  } else
    return NextResponse.json({ error: "invalid format OR no data in body" });
}
